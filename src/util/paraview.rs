...
pub fn paraview_write_vtu<P>(input: &FemInput, state: &FemState, full_path: &P) -> Result<(), StrError>
where
    P: AsRef<OsStr> + ?Sized,
{
    let mesh = input.mesh;
    ...
    if enabled_dofs.contains(&Dof::Ux) {
        ...
        for point in &mesh.points {
            let ux = match input.equations.eq(point.id, Dof::Ux).ok() {
                Some(eq) => state.uu[eq],
                None => 0.0,
            };
            let uy = match input.equations.eq(point.id, Dof::Uy).ok() {
                Some(eq) => state.uu[eq],
                None => 0.0,
            };
            let uz = match input.equations.eq(point.id, Dof::Uz).ok() {
                Some(eq) => state.uu[eq],
                None => 0.0,
            };
    ...
    for dof in &not_displacement_dof {
        ...
        .unwrap();
        for point in &mesh.points {
            let value = match input.equations.eq(point.id, *dof).ok() {
                Some(eq) => state.uu[eq],
                None => 0.0,
            };
...
mod tests {
    ...
    use crate::fem::{FemInput, FemState, DEFAULT_OUTPUT_DIR};
    ...
    fn write_vtu_works() {
        ...
        let path = format!("{}/{}-{:0>20}.vtu", DEFAULT_OUTPUT_DIR, fn_stem, 0);
        paraview_write_vtu(&input, &state, &path).unwrap();
