...
pub struct DataSPO {
    ...
    pub plast_apex_epbar: Vec<Vec<Vec<f64>>>,
}
...
impl ReferenceDataSPO {
        ...
        P: AsRef<OsStr> + ?Sized,
    {
        ...
        let file = File::open(&path).map_err(|_| "ReferenceDataSPO: file not found")?;
        let reader = BufReader::new(file);
        let data = serde_json::from_reader(reader).map_err(|_| "ReferenceDataSPO: deserialize failed")?;
        Ok(data)
    }
}
