...
pub(crate) fn callback_ode_e(dydt: &mut Vector, _t: f64, y: &Vector, a: &mut Args) -> Result<(), StrError> {
    // copy {y}(t) into σ
...
pub(crate) fn callback_ode_ep(dydt: &mut Vector, _t: f64, y: &Vector, a: &mut Args) -> Result<(), StrError> {
...
pub(crate) fn callback_intersect(stats: &Stats, _h: f64, t: f64, y: &Vector, a: &mut Args) -> Result<bool, StrError> {
    // reset the counter
...
pub(crate) fn callback_history_e(_stats: &Stats, _h: f64, t: f64, y: &Vector, a: &mut Args) -> Result<bool, StrError> {
...
pub(crate) fn callback_history_ep(_stats: &Stats, _h: f64, t: f64, y: &Vector, a: &mut Args) -> Result<bool, StrError> {
    if let Some(h) = a.history_eep.as_mut() {
        // split {y}(t) into σ and z
        y.split2(
            a.state.stress.vector_mut().as_mut_data(),
