...
impl Axis {
    ...
    fn calc(&self, states: &[LocalStateOld]) -> Vec<f64> {
        match self {
...
impl StressStrainPlot {
    ...
    pub fn draw<F>(&mut self, x_axis: Axis, y_axis: Axis, states: &[LocalStateOld], mut config: F)
    where
        F: FnMut(&mut Curve),
    {
    ...
    pub fn draw_oct_projection<F>(&mut self, states: &[LocalStateOld], mut extra: F) -> Result<(), StrError>
    where
        F: FnMut(&mut Curve),
    {
        let n = states.len();
    ...
    pub fn draw_3x2_mosaic_struct<F>(&mut self, states: &[LocalStateOld], mut extra: F)
    where
    ...
    /// * `states` -- the stress and strain points
    /// * `extra` -- is a function `|curve, row, col| {}` to configure the curve
    pub fn draw_2x2_mosaic_struct<F>(&mut self, states: &[LocalStateOld], mut extra: F)
    where
        F: FnMut(&mut Curve, usize, usize),
    {
        let percent = true;
