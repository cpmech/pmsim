use super::{LocalStateOld, StressStrainTrait, VonMises};
...
pub trait PlasticityTrait: StressStrainTrait {
    ...
    fn yield_function(&self, state: &LocalStateOld) -> Result<f64, StrError>;

    /// Calculates the plastic potential function g
    fn plastic_potential(&self, state: &LocalStateOld) -> Result<(), StrError>;

    /// Calculates the hardening coefficients H_i corresponding to the incremental hardening model
    fn hardening(&self, hh: &mut Vector, state: &LocalStateOld) -> Result<(), StrError>;

    /// Calculates the derivative of the yield function w.r.t stress
    fn df_dsigma(&self, df_dsigma: &mut Tensor2, state: &LocalStateOld) -> Result<(), StrError>;

    /// Calculates the derivative of the plastic potential w.r.t stress
    fn dg_dsigma(&self, dg_dsigma: &mut Tensor2, state: &LocalStateOld) -> Result<(), StrError>;

    /// Calculates the derivative of the yield function w.r.t internal variables
    fn df_dz(&self, df_dz: &mut Vector, state: &LocalStateOld) -> Result<(), StrError>;

    /// Calculates the elastic compliance modulus
    fn elastic_compliance(&self, cce: &mut Tensor4, state: &LocalStateOld) -> Result<(), StrError>;

    /// Calculates the elastic rigidity modulus
    fn elastic_rigidity(&self, dde: &mut Tensor4, state: &LocalStateOld) -> Result<(), StrError>;
}
...
impl Plasticity {
    ...
    fn set_nonlin_elast_params(&mut self, state: &LocalStateOld) {
    ...
    pub fn elastic_compliance(&mut self, state: &LocalStateOld) -> Result<(), StrError> {
    ...
    pub fn elastic_rigidity(&mut self, state: &LocalStateOld) -> Result<(), StrError> {
        match self.params_nonlin_elast.as_ref() {
            Some(_) => {
                self.set_nonlin_elast_params(state);
    ...
    pub fn elastoplastic_compliance(&mut self, cc: &mut Tensor4, state: &LocalStateOld) -> Result<(), StrError> {
        // derivatives
...
mod tests {
    ...
    fn elastoplastic_rigidity_works() {
        ...
        let mut state = LocalStateOld::new(config.mandel, 1, false);
