...
mod tests {
    ...
    fn update_to_yield_surface(ideal: &Idealization, model: &mut VonMisesSoft, lode: f64) -> LocalState {
        // elastic parameters
    ...
    fn initialize_internal_values_works() {
        ...
        let param = StressStrain::VonMisesSoft {
            young: YOUNG,
            poisson: POISSON,
            hh: HH,
            z_ini: Z_INI,
        ...
        let model = VonMisesSoft::new(&ideal, &param, &settings).unwrap();
    ...
    fn update_stress_works_elastic() {
        for ndim in [2, 3] {
            ...
            let settings = Settings::new();
            let mut model = VonMisesSoft::new(&ideal, &param, &settings).unwrap();
