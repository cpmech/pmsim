...
impl StressStrainPath {
    ...
    pub fn push_stress(&mut self, sigma: &Tensor2, strain_driven: bool) {
        ...
        let mut state = LocalStateOld::new(self.mandel, 0, with_optional); // 0 => no internal variables
    ...
    pub fn follow_strain(&self, model: &mut dyn StressStrainTrait) -> Vec<LocalStateOld> {
        assert!(self.states.len() > 0);

        // initial model state
        let n_internal_values = model.n_internal_values();
        let with_optional = true;
        let mut state = LocalStateOld::new(self.mandel, n_internal_values, with_optional);
