...
impl fmt::Display for StressStrainPath {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for i in 0..self.stresses.len() {
            ...
            write!(f, "    {}σ = {:?}\n", a, sigma.vec.as_data()).unwrap();
            write!(f, "    {}ε = {:?}\n", b, epsilon.vec.as_data()).unwrap();
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn new_works() {
        ...
        path.push_stress_with_oct_invariants(1.0, 9.0, 1.0, true).unwrap();
        path.push_stress_with_oct_invariants(2.0, 18.0, 1.0, true).unwrap();
        path.push_stress_with_oct_invariants(3.0, 36.0, 1.0, true).unwrap();
        println!("{}", path);
    }
}
