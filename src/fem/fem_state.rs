...
impl FemState {
    ...
    pub fn new(mesh: &Mesh, base: &FemBase, essential: &Essential, config: &Config) -> Result<FemState, StrError> {
        ...
        let neq_total = if config.lagrange_mult_method {
            ...
            base.equations.size() + n_lagrange
        } else {
            base.equations.size()
        };
...
mod tests {
    ...
    fn new_works_mixed() {
        ...
        assert_eq!(state.duu.dim(), base.equations.size());
        assert_eq!(state.uu.dim(), base.equations.size());
    }
    ...
    fn new_works_diffusion() {
        ...
        assert_eq!(state.duu.dim(), base.equations.size());
        assert_eq!(state.uu.dim(), base.equations.size());
        assert_eq!(state.vv.dim(), base.equations.size());
        assert_eq!(state.aa.dim(), 0);
        assert_eq!(state.uu_star.dim(), base.equations.size());
        assert_eq!(state.vv_star.dim(), base.equations.size());
    ...
    fn new_works_solid_and_rod() {
        ...
        assert_eq!(state.uu.dim(), base.equations.size());
        assert_eq!(state.vv.dim(), base.equations.size());
        assert_eq!(state.aa.dim(), base.equations.size());
        assert_eq!(state.uu_star.dim(), base.equations.size());
        assert_eq!(state.vv_star.dim(), base.equations.size());
        assert_eq!(state.aa_star.dim(), base.equations.size());
    }
