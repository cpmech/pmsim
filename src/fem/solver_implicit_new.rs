...
pub struct SolverImplicitNew<'a> {
...
impl<'a> SolverImplicitNew<'a> {
    ...
    pub fn solve(&mut self, state: &mut FemState, file_io: &mut FileIo) -> Result<(), StrError> {
        ...
        for timestep in 0..config.n_max_time_steps {
            ...
            // reset accumulated primary values
            state.duu.fill(0.0);
...
mod tests {
    ...
    fn new_captures_errors() {
        ...
        assert_eq!(
            SolverImplicitNew::new(&mesh, &base, &config, &essential, &natural).err(),
    ...
    fn run_captures_errors() {
        ...
        let essential = Essential::new();
        let natural = Natural::new();
        let mut solver = SolverImplicitNew::new(&mesh, &base, &config, &essential, &natural).unwrap();
